...
gen_psa_params <- function(n_psa, seed = 1, pr = 0,
  ...
  # From Kwon et al 2020
  # 31 * 0.92
  psa_params$p_cure_wo_emb_6mo <- r_p(p_r(rbeta(n_psa, 29 + pr, 44 - 29 + pr), 6))
  # 31 * 0.08
  psa_params$p_cure_wo_emb_late <- r_p(p_r(rbeta(n_psa,  2.48 + pr, 
                                                 44 - 2.48 + pr), 6))
  # From Miwa et al
  ...
  # Intolerance of azithromycin (assume same as p_cure_x)
  psa_params$p_cure_wo_azi_6mo <- r_p(p_r(rbeta(n_psa, 154 + pr, 180 - 154 + pr), 6))
  psa_params$p_cure_wo_azi_late <- r_p(p_r(rbeta(n_psa, 65 + pr, 224 - 65 + pr), 6))
  ...
  # u5 <- beta_params((0.966-0.949), sqrt(0.015^2 + 0.017^2))
  # psa_params$DisU_obsT <- rbeta(n_psa, u5$alpha, u5$beta)
  
  ## From Kwak 2020 (calculating disutility of progression)
  # sgrq_eq5d <- function(sgrq, male = T) {
  #   return(0.9617 - 0.0013 * sgrq - 1e-04 * sgrq^2 + 0.0231 * male)
  # }
  ...
  # ## SD
  # # UCI = 29; LCI = 17
  # (sgrq_eq5d(17) - sgrq_eq5d(29))/3.92
  # # 0.01806122
  # 
  # ## Main effect (needing treatment)
  # sgrq_eq5d(28)
  # # 0.87
  # 
  # ## SD
  # # UCI = 35; LCI 21
  # (sgrq_eq5d(21) - sgrq_eq5d(35))/3.92
  # # 0.02464286
  
  u5 <- beta_params((0.902-0.87), sqrt(0.01806122^2 + 0.02464286^2))
  psa_params$DisU_obsT <- rbeta(n_psa, u5$alpha, u5$beta)
